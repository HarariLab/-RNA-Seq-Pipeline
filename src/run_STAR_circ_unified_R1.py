...
--outSAMtype circ R1 default: None
...
--outFilterMismatchNmax: 999 (for both reads)
--outFilterMismatchNmax: 2 (for R1)

--chimScoreSeparation: 10 (default, but want captured)

--genomeLoad default=NoSharedMemory
--genomeLoad circRNA: LoadAndKeep - have to use NoSharedMemory for 2-pass mode

--twopassMode default: Basic (for both reads)
--twopassMode default: None (for R1)
...
parser.add_argument('--alignIntronMin', default='21')
...
parser.add_argument('--peOverlapMMp', default='0.01')
...
parser.add_argument('--quantMode', default='-', help='Outputs read counts, and a BAM with reads in transcriptome coordinates')
parser.add_argument('--outSAMtype', default='None')
...
parser.add_argument('--chimOutType', default=['Junctions'], nargs='+', help='')
parser.add_argument('--chimMainSegmentMultNmax', default='10', help='')
parser.add_argument('--genomeLoad', default='LoadAndKeep')
...
parser.add_argument('--limitBAMsortRAM', default='50000000000', help='Specific for circRNA R1 and R2')
...
parser.add_argument('--chimScoreSeparation', default='10', help='RNAseq = 10; default, but want captured') #KM
...
cmd +=' --outFilterMultimapNmax '+args.outFilterMultimapNmax\
    ...
    +' --chimScoreSeparation '+args.chimScoreSeparation\
    +' --limitSjdbInsertNsj '+args.limitSjdbInsertNsj\
    +' --peOverlapMMp '+args.peOverlapMMp\
...
cmd += ' --readFilesCommand samtools view -h' \
    ...
    +' --limitGenomeGenerateRAM '+args.limitGenomeGenerateRAM+' --limitBAMsortRAM '+args.limitBAMsortRAM+' --genomeChrBinNbits '+args.genomeChrBinNbits
...
#if args.outSAMtype is not None:
     #cmd += ' --outSAMtype '+' '.join(args.outSAMtype)
#else:
    #cmd += ' --outSAMtype None'
...
with cd(args.outputDir):
    ...
    # sort BAM (use samtools to get around the memory gluttony of STAR) - deprecated, this step doesn't produce a bam
