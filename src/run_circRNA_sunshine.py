...
if (args.read_type == "PE"):
    # Read 1
    cmd = 'samtools view -h -b -f 0x0040 '+os.path.join(local_raw,args.sample_id+'_unmapped.bam') + ' > ' \
    + os.path.join(args.storage_circ,args.sample_id,args.sample_id) + "_1.bam"
    subprocess.check_call(cmd, shell=True, executable='/bin/bash')
    logger.info(' cmd * '+cmd)

    # Read 2
    cmd = 'samtools view -h -b -f 0x0080 '+os.path.join(local_raw,args.sample_id+'_unmapped.bam')+' > ' \
...
+args.star_index +' ' +os.path.join(args.storage_circ,args.sample_id,args.sample_id +'_unified ') + os.path.join(local_raw,args.sample_id+'_unmapped.bam')
