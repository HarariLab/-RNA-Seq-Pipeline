...
bams = storage.get('bams'); print "bams: {}".format(bams); #bamout = str(bams); print"bamout" #, end=''
outdir = paths.get('outdir'); print "outdir: {}".format(outdir); #print('outdir: ', end=''); print(outdir)
storage_bams = os.path.join(outdir, args.study, bams, args.tissue); print "storage_bams: {}".format(storage_bams); #print('storage_bams: ', end=''); print(storage_bams)

circ = storage.get('circ'); print "circ: {}".format(circ); #print('circ: ', end=''); print(circ)
storage_circ = os.path.join(outdir, args.study, circ, args.tissue); print "storage_circ: {}".format(storage_circ); #print('storage_circ: ', end=''); print(storage_circ)
...
print "local_genome_dir: {}".format(local_genome_dir);
...
dcc = storage.get('dcc'); print "dcc: {}".format(dcc) #print('dcc: ', end=''); print(dcc)
storage_dcc = os.path.join(outdir, args.study, dcc, args.tissue); print "storage_dcc: {}".format(storage_dcc) #print('storage_dcc: ', end=''); print(storage_dcc)

dcc_in = storage.get('dcc_in'); #print "dcc_in: {}".format(dcc_in) print('dcc_in: ', end=''); print(dcc_in)
storage_dcc_in =os.path.join(outdir, args.study, dcc_in, args.tissue); #print('storage_dcc_in: ', end=''); print(storage_dcc_in)
storage_tmp = os.path.join(outdir, args.study, dcc_in, args.tissue,'_tmp_DCC'); print "storage_tmp: {}".format(storage_tmp)
...
if not os.path.exists(storage_tmp):
    os.makedirs(storage_tmp)

#print('output destinations: '+'\n\t'+storage_dcc+'\n\t'+storage_dcc_in)

# Get the list of folders = samples
samples = os.listdir(storage_circ) #samples = os.listdir(path=os.path.join(storage_circ)) # TURN ON FOR MGI
...
for sample_id in samples:
    ...
    logger.info(' cmd * '+cmd)
    #print "sample_id: {}".format(sample_id) #print('sample_id: ' +sample_id)
    #print"cmd: {}".format(cmd) #print(cmd)
    subprocess.check_call(cmd, shell=True, executable='/bin/bash')

    logger.info('DCC input files: bamfiles')
    cmd = 'ls -d '+ os.path.join(storage_bams,sample_id)+'.Aligned.sortedByCoord.out.md.bam >> ' + os.path.join(storage_dcc_in)+'/bam_files' # was _fixed
...
if (end == "PE"):
    ...
    cmd = '/usr/bin/time -v DCC @' +os.path.join(storage_dcc_in)+'/samplesheet -mt1 @' +os.path.join(storage_dcc_in)+'/mate1 -mt2 @'+os.path.join(storage_dcc_in)+'/mate2 -T 4 -D -R ' +repeat_mask +' -an '+annot +' -Pi -F -M -Nr 1 1 -fg -k -G -A ' +ref+' -B @'+os.path.join(storage_dcc_in)+'/bam_files ' +'-O '+os.path.join(storage_dcc) +' -t ' +os.path.join(storage_tmp)
...
# SE only
    ...
    for sample_id in samples:
        ...
        if sample_id =='samples.txt':
            continue
        logger.info('DCC input files: read')
    ...
    logger.info('Starting DCC for: '+ args.study)
    os.chdir(storage_dcc) #TURN ON FOR MGI
    cmd = '/usr/bin/time -v DCC @' +os.path.join(storage_dcc_in)+'/samplesheet -mt1 @' +os.path.join(storage_dcc_in)+'/read -T 2 -D -N -R ' +repeat_mask +' -an '+annot +' -F -M -Nr 1 1 -fg -k -G -A ' +ref+' -B @'+os.path.join(storage_dcc_in)+'/bam_files ' +'-O '+os.path.join(storage_dcc) +' -t ' +os.path.join(storage_tmp)
...
# "outa: {}".format(outa); #print"outa: {}".format(outa)
...
print "outf: {}".format(outf);#print(outf)
shutil.rmtree(outf) # turn on when working
