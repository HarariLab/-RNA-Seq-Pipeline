...
#
        for i, rg in enumerate(unprocessed_list):
            ...
            repeat_mask = os.path.join(local_genome_dir, os.path.split(getValue(paths,'genome/repeat_mask'))[1])
            read_len = data.get('read_len')
            read_type = data.get('read_type')
            tmp_dir = os.path.join(paths.get('tmp'),sample_id)
            storage_aligned = os.path.join(output_dir, getValue(paths,'storage/aligned'),tissue) #, sample_id
            storage_bams = os.path.join(output_dir, getValue(paths,'storage/bams'),tissue)
            storage_qc = os.path.join(output_dir,  getValue(paths,'storage/qc'),tissue) #, sample_id
            storage_quant = os.path.join(output_dir, getValue(paths,'storage/quant'),tissue) #, sample_id
            storage_tin = os.path.join(output_dir, getValue(paths,'storage/tin'),tissue) #, sample_id
            storage_circ = os.path.join(output_dir, getValue(paths,'storage/circ'),tissue) #, sample_id
            json = args.json
            #print (job_name,file_type, host, src, sample_id )

            cmd="bsub -Is -q research-hpc -J "+job_name \
            +" -M 50000000 " \
            +" -R 'select [mem>50000 && gtmp > 15] rusage[mem=50000, gtmp=15]'" \
            +" -a 'docker(de1malo1bonum/rnaseq_km:unified.0.1.1)'" \
            +" /bin/bash -c '"+os.path.join(src,"pipeline_unified.py")+" " \
            +" ".join([host, src, path_to_local_seq_file, file_type, cohort, tissue, sample_id, ref, star_index, salmon_index, \
              ref_flat, ribosomal_int, annot, repeat_mask, read_len, read_type, tmp_dir, output_dir, storage_aligned, \
              storage_bams, storage_qc, storage_quant, storage_tin, storage_circ, json])+"'"

            print(cmd)
        #    subprocess.Popen(cmd,shell=True) # MGI
